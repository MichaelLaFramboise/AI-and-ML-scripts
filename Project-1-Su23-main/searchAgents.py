...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    distances = [bfs_distance(position, corner, walls) for corner in unvisited_corners]
    return max(distances)
from util import Queue
def bfs_distance(start, end, walls):
    queue = Queue()
    queue.push((start, 0))
    visited = set()
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if not foodList:  # no food left, we're at the goal
        return 0

    # Use the maximum Manhattan distance from the current position to any food pellet
